/**
 * Fetch LSP3 (profile), LSP4, LSP26, LSP27 metadata from Universal Profiles.
 * Copied from frontend-handshake for LUKSO profile resolution.
 */
...
export async function getLSP3Metadata(provider: Provider, upAddress: string): Promise<LSP3ProfileJSON | null> {
  try {
...
export async function getLSP4Metadata(provider: Provider, upAddress: string): Promise<LSP4Metadata | null> {
  try {
    const normalized = getAddress(upAddress.trim());
    const up = new Contract(normalized, ERC725Y_ABI, provider);
...
export async function getLSP26Data(provider: Provider, upAddress: string): Promise<LSP26Data | null> {
  try {
    const normalized = getAddress(upAddress.trim());
    const up = new Contract(normalized, ERC725Y_ABI, provider);
...
export async function getLSP27Data(provider: Provider, upAddress: string): Promise<LSP26Data | null> {
  try {
    const normalized = getAddress(upAddress.trim());
    const up = new Contract(normalized, ERC725Y_ABI, provider);
...
export async function getProfileData(provider: Provider, address: string): Promise<ProfileData | null> {
  try {
...
export async function updateLSP4Description(
  ...
  void newDescription.slice(0, 1000).replace(/<script[^>]*>.*?<\/script>/gi, "").trim();
  void _signer;
  void _upAddress;
  throw new Error("LSP4 description update not yet implemented");
}
