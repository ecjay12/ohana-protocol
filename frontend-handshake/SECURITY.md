...
- **APIs**: `/api/vouches`, `/api/github-attestation`, and `/api/vouch-graph` are GET-only. `chainId` is validated where applicable; `/api/vouch-graph` currently returns a public stub graph for layout until an indexer is connected.
